...
class AlchemyInterface {
    ...
    dateToNotes = async (ethereum, from, date) => { 
        const params = {
            ...
            data: this.interface.encodeFunctionData("dateToNotes", [ethers.BigNumber.from(date)])
        }
        return await ethereum.request({method: 'eth_sendTransation', params: params})
    }
    ...
    getNotes = async (ethereum, from, number) => {
        const params = {
            from: from,
            to: this.smartContract,
            data: this.interface.encodeFunctionData("getNotes", [ethers.BigNumber.from(number)])
        }
        return await ethereum.request({method: 'eth_sendTransation', params: params})
    }
    ...
    uploadNote = async (ethereum, from, date, noteAddress) => {
        const params = {
            from: from,
            to: this.smartContract,
            data: this.interface.encodeFunctionData("uploadNote", [date.toString(), noteAddress])
        }
        return await ethereum.request({method: 'eth_sendTransation', params: params})
    }
